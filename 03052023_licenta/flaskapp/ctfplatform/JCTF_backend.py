...
from ctfplatform.utils import append_new_line, deployment_template_path, service_template_path, namespace_template_path, secret_template_path, ready_manifests_folder_path
...
def create_kube_manifests(name, port, image, username):
    try:
        ...
        namespace_file_path = create_namespace_from_template(namespace_template_path, custom_namespace)
        secret_file_path = create_secret_from_template(secret_template_path, custom_namespace)
        deployment_file_path = create_deployment_from_template(deployment_template_path, custom_namespace, image, port)
        service_file_path = create_service_from_template(service_template_path, custom_namespace, port )
        custom_name_folder = get_ctf_folder(ready_manifests_folder_path, custom_namespace)
        ##apply manifes files 
        
        res = kube_interaction_inst.kube_apply_directory(custom_name_folder)
        if res == True:
            deployments_info = kube_interaction_inst.kube_list_deployments(name=name, namespace=custom_namespace)
            services_info = kube_interaction_inst.kube_list_services(name=name, namespace=custom_namespace)
            ...
            insert_kube_manifests(name, image, deployment_file_path, service_file_path, namespace_file_path, replicas[0], status[0], nodePort[0], external_ip)
        else:
...
def create_deployment_from_template(deployment_template_path, name, image, port):
    try:
        ...
        if not os.path.exists(deployment_file_path):
            ...
            #registry_url = kube_interaction_inst.kube_get_docker_registry()
            #manifest_url = f"https://{registry_url}/{image}"
            ...
            append_new_line("logs.txt", f"Updating {name}-deployment.yaml...")
            deployment_content = deployment_content.replace('<TBD_IMAGE>', manifest_url)
            append_new_line("logs.txt", f"{name}-deployment.yaml already exists: image updated with {manifest_url} ")
