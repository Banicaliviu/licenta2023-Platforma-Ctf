...
def docker_pull(imageName):
    try:
        ...
        append_new_line(
                "logs.txt", f"Image {imageName} pulled successfully "
            )
        return True
    except docker.errors.APIError as e:
        append_new_line(
                "logs.txt", f"Error at pulling image {imageName}: {e}"
            )
        return False

def updateImageTag(imageName, tag):
    append_new_line(
        "logs.txt", f"Updating image {imageName} tag..."
    )
    append_new_line(
        "logs.txt", f"Tag to be added {tag} tag..."
    )
    
    new_imageName=f"{tag}/{imageName}"
    append_new_line(
        "logs.txt", f"Image updated: {new_imageName}"
    )
...
def docker_push(imageName):
    ...
    try:
        ...
        append_new_line(
                "logs.txt", f"Image {imageName} pushed successfully "
            )
        return True
    except docker.errors.APIError as e:
        append_new_line(
                "logs.txt", f"Error at pushing image {imageName}: {e}"
            )
        return False        
