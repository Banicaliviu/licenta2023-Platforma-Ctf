...
class kubernetes_interaction:
    ...
    def kube_apply_directory(self, directory):
        try:
            ...
            if namespace_file:
                ...
                append_new_line("logs.txt", f"Successfully applied file {namespace_file} to Kubernetes cluster")
            if secret_file:
                utils.create_from_yaml(self.apiClient, secret_file)
                append_new_line("logs.txt", f"Successfully applied file {secret_file} to Kubernetes cluster")
            for file_path in other_files:
                utils.create_from_yaml(self.apiClient, file_path)
                append_new_line("logs.txt", f"Successfully applied file {file_path} to Kubernetes cluster")

            return True
        except Exception as e:
            append_new_line("logs.txt", f"Error applying files in directory {directory} to Kubernetes cluster: {e}")
            return False
    ...
    def kube_delete_file(self,file):
        try:
            ...
            with open(file, 'r') as f:
                ...
                self.apiClient.delete_namespaced_pod(body['metadata']['name'], 'ctfspace')
            append_new_line("logs.txt", f"Successfully deleted file {file} from Kubernetes cluster")
            return True
        except Exception as e:
            append_new_line("logs.txt", f"Error deleting file {file} from Kubernetes cluster: {e}")
            return False
    ...
    def kube_list_deployments(self, name, namespace):
        try:
            ...
            append_new_line("kube-logs.txt", f"Listing deployments in {namespace} namespace with name {name}...")
    ...
    def kube_list_services(self, name, namespace):
        try:
            api_instance = client.CoreV1Api(self.apiClient)
            append_new_line("kube-logs.txt", f"Listing services in {namespace} namespace with name {name}...")
    ...
    def kube_get_chartmuseum(self):
        ...
        if port_number is None:
            append_new_line(
                "kube-logs.txt", "Unable to retrieve port for Chartmuseum."
            )
            return
    ...
    def kube_get_docker_registry(self):
        ...
        if not pods.items:
            append_new_line(
                "kube-logs.txt", f"No docker-registry pod found in {namespace_selector} namespace."
            )
            return
