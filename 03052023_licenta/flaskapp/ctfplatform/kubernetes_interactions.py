...
class kubernetes_interaction:
    ...
    def kube_get_chartmuseum(self):
        ...
        pods = api_instance.list_namespaced_pod(namespace=namespace_selector, label_selector=label_selector)
        if not pods.items:
            append_new_line("kube-logs.txt", "No chartmuseum pods found in ctfspace namespace.")
            return
        ...
        if port_number is None:
            append_new_line("kube-logs.txt", "Unable to retrieve the port number for Chartmuseum.")
            return
    ...
    def kube_add_chartmuseum_repository(self, cm_url):
        try:
            ...
            if result.returncode == 0:
                ...
                for repo in repo_list:
                    if repo["name"] == repo_name:
                        append_new_line("logs.txt", f"Repository '{repo_name}' already exists")
                        return True
            ...
            append_new_line("logs.txt", f"Failed to add repository '{repo_name}': {e.stderr}")
            return 0
