...
class PrintHandler(RequestHandler):
    ...
    async def __print(self):
        ...
        try:
            ...
            self.__reject(
                "[print_img:exception] service failed to process "
        ...
        try:
            ...
            access_log.exception("An exception occurred...")
            self.__reject(
                "[__save_secret:exception] service failed to save "
    ...
    async def __serial_number(self):
        ...
        if data[0:3] == b"sn=":
            ...
            try:
                ...
                self.__reject(
                    "[__load_secret:exception] service failed to "
            ...
            if saved_secret is None:
                self.__reject(
                    "Too late, you must submit your serial-number "
            ...
            if secret == saved_secret:
                ...
                self.__accept(
                    "Good job ! DO NOT LEAK SECRET FILES BY "
            ...
            access_log.info(
                "received secret is [{}] and saved_secret is [{}]; mismatch!".format(
                    secret, saved_secret
                )
            )
    ...
    return Application(
        [
            (
                r"/serial-number",
                SerialNumberHandler,
                dict(redis_host=redis_host, redis_port=redis_port, flag=flag),
            ),
            (
                r"/print",
                PrintHandler,
                dict(redis_host=redis_host, redis_port=redis_port),
            ),
            (r"/", MainHandler),
        ],
        debug=debug,
...
if __name__ == "__main__":
    ...
    p.add_argument("-d", "--debug", action="store_true", help="turns on debug mode.")
    p.add_argument(
        "-c", "--config", default=".mkctf.yml", help="mkctf configuration file."
    )
    args = p.parse_args()
