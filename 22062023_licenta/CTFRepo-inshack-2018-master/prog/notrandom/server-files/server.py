...
class ClientThread(Thread):
    def __init__(self, ip, port, conn, flag):
        ...
        self.flag = flag
        self.conn.send(
            'Hello and welcome to "Will you get your degree" challenge !  \n'.encode()
        )
    ...
    def run(self):
        while self.init_credit > 0:
            try:
                ...
                self.conn.send(
                    (
                        "To be sure we are fair, here is the commitment of our future jackpot %s\n"
                        % (commit_result[0])
                    ).encode()
                )
                self.conn.send(
                    "Pick your magic number between 0 and 2**10 : \n".encode()
                )
                ...
                self.conn.send(
                    (
                        "Commitment values : {0:04d} + {1:10d}\n".format(
                            commit_result[1], commit_result[2]
                        )
                    ).encode()
                )
                self.conn.send(
                    ("You have %d credits remaining\n" % (self.init_credit)).encode()
                )
