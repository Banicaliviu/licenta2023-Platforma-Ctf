...
while True:
    ...
    if user_command[0] == "ls":
        for opt in user_command[1:]:
            if not opt.startswith("-") and opt not in {".", "./"}:
                print(
                    "Sorry, you can only see the current directory (`ls -la` for example)"
                )
                skip = True
    if user_command[0] == "openstack":
        ...
        if len(user_command) == 2 and user_command[1] in {"--help", "-h", "--version"}:
            ...
            len(user_command) == 3
            and user_command[2] in {"--help", "-h", "--version"}
            and user_command[1] == "--debug"
        ):
            # ok
            pass
