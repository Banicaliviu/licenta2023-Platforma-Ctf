...
def random_byte_str(n):
    return b"".join(
        random.SystemRandom().choice(string.ascii_letters + string.digits).encode()
        for _ in range(n)
    )
...
def extract_user_info(cookie):
    ...
    assert (
        sign(PRIVATE_KEY, salt.encode(), yaml_document) == signature
    ), "Stop it, you clever(ish) hax0r!"
    # Never trust user input
    ...
    assert (
        len({"name", "insa_coins", "valid_until"} - set(user.keys())) == 0
    ), "Expected keys were not present"
...
def home():
    ...
    resp = make_response(
        render_template("index.html", insa_coins=user["insa_coins"], name=name)
    )
...
def pay():
    ...
    if f == "premium" and cost != 500:
        return render_template(
            "error.html",
            error="Those files look precious to you, It will cost a lot to decrypt them.",
        )

    user_cookie = request.cookies.get("user")
    if not user_cookie:
        return render_template(
            "error.html", error="you has no coin, pleaze go by some!"
        )
    try:
        user = extract_user_info(user_cookie)
    except Exception as e:
    ...
    if insa_coins < cost:
        return render_template(
            "error.html",
            error="You don't have enough INShAcoins to decrypt this file, please go buy some",
        )
    insa_coins -= cost
...
def change_name():
    ...
    resp = make_response(
        "Thanks <i>{}</i>, now you can <a href='/'>pay us</a>.".format(user["name"])
    )
    resp.set_cookie("user", user_cookie)
    return resp
