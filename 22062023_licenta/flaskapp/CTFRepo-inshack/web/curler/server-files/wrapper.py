...
def change_config():
    ...
    assert isinstance(
        new_config_value, type(fetch_options[config]["value"])
    ), "Wrong type for new config value"
    assert (
        new_config_value in fetch_options[config]["allowed_values"]
    ), "New config value not in allowed set"
...
def fetch():
    ...
    params = bson.dumps({"options": options})
...
while True:
    ...
    print(
        "Fetcher options: {}".format({k: v["value"] for k, v in fetch_options.items()})
    )
    print()
